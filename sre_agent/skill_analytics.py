...
def record_skill_invocation(
    *,
    ...
    try:
        ...
        get_skill_analytics_repo().record_invocation(
...
def get_skill_stats(days: int = 30) -> dict:
    ...
    try:
        ...
        # Per-skill stats
        skill_rows = repo.fetch_skill_stats(days)

        # Top tools per skill in a single query (avoids N+1)
        all_tool_rows = repo.fetch_all_skill_tools(days)
        ...
        # Handoff flows
        handoff_rows = repo.fetch_handoffs(days)
...
def get_skill_trend(skill_name: str, days: int = 30) -> dict:
    ...
    try:
        ...
        rows = get_skill_analytics_repo().fetch_skill_trend(skill_name, days)
...
def get_skill_user_breakdown(skill_name: str, days: int = 30) -> list[dict]:
    ...
    try:
        ...
        rows = get_skill_analytics_repo().fetch_skill_user_breakdown(skill_name, days)
        return [
...
def update_skill_feedback(session_id: str, feedback: str) -> None:
    ...
    try:
        ...
        get_skill_analytics_repo().update_feedback(session_id, feedback)
    except Exception:
